...
report={'patch_intrusion_into_original_airway_mm3':[p.common(core).Volume for p in patches], 'new_material_inside_original_airway_mm3':final.cut(base).common(core).Volume,'wall_loss_mm3':D.getObject('OriginalWallSkin').Shape.cut(final).Volume,'core_valid':core.isValid(),'core_solids':len(core.Solids),'duct_valid':final.isValid(),'arms_difference_mm3':{},'fit_gaps_mm':{}}
...
report['pass']=all(x<1e-6 for x in report['patch_intrusion_into_original_airway_mm3']) and report['new_material_inside_original_airway_mm3']<1e-6 and report['wall_loss_mm3']<1e-6 and all(x<1e-6 for x in report['arms_difference_mm3'].values()) and all(abs(x-.3)<1e-5 for x in report['fit_gaps_mm'].values()) and core.isValid() and final.isValid()
...
A.Console.PrintMessage('Airway audit PASS: patches add zero intrusion into the original lofted passage; wall continuous; arms frozen; six gaps 0.30 mm.\n')
