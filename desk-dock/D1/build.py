"""D1 review CAD. X insertion axis; +Y lid/fans; -Y underside/intakes; +Z up.
...
# Rounded two-piece footprint. The lid-side discharge skirt blocks return to
# the rear intake face and gives air a dedicated front exit at y=160.
for i,(x0,x1,fx) in enumerate([(-7,W/2,84),(W/2+.30,W+7,269.68)],1):
    length=x1-x0
    flange=rb(x0,-24,30,length,188,3,8)
    flange=flange.cut(hole((0,0,1),(fx,91,29),56.5,6))
    # Curved Y-Z section: throat under hinge, smooth roof into fan suction side.
    outer=(cq.Workplane('YZ',origin=(x0,0,0)).moveTo(-24,33)
        .lineTo(-24,41).threePointArc((-21,48),(-13,50))
        .lineTo(11,50).threePointArc((30,49),(45,48))
        .lineTo(155,48).threePointArc((162,45),(164,39))
        .lineTo(164,33).close().extrude(length))
    inner=(cq.Workplane('YZ',origin=(x0+2.4,0,0)).moveTo(-21.6,32.9)
        .lineTo(-21.6,40).threePointArc((-18,46),(-12,47.6))
        .lineTo(11,47.6).threePointArc((30,46.6),(45,45.6))
        .lineTo(154,45.6).threePointArc((159,43),(161.6,38))
        .lineTo(161.6,32.9).close().extrude(length-4.8))
    roof=outer.cut(inner)
    # Under-hinge mouth overlaps each plenum; no suction openings at base grille.
    ma=max(24,x0+3);mb=min(W-24,x1-3)
    roof=roof.cut(box(ma,-7,44,mb-ma,17,15))
    # Mounting bosses outside swept blade disk; four 105 mm-pattern bores.
    for xx in [fx-52.5,fx+52.5]:
      for yy in [38.5,143.5]:
        roof=roof.union(hole((0,0,1),(xx,yy,33),5,13))
        roof=roof.cut(hole((0,0,1),(xx,yy,29),2.2,23))
        flange=flange.cut(hole((0,0,1),(xx,yy,29),2.2,6))
    # Back skirt and side legs guide the fan outlet toward the lid/front side.
    skirt=rb(x0,-24,2,length,188,28,8).cut(box(x0+4,-20,1,length-8,181,31))
    skirt=skirt.cut(box(x0+5,158,2,length-10,10,24))
    for xx in [x0+12,x1-12]:
        skirt=skirt.union(box(xx,158,2,3,6,28))
    add(f'{i:02}_suction_roof',roof)
    add(f'{i:02}_fan_deck',flange)
    add(f'{i:02}_outlet_skirt',skirt)
    for xx in [x0+7,x1-19]:
      for yy in [-20,142]:add(f'{i:02}_foot_{xx:.1f}_{yy}',rb(xx,yy,0,12,16,2,3),(31,34,37))
    ...
    # Slender underside finger guard; protected between skirt and desktop.
    guard=cq.Workplane('XY').center(fx,91).circle(58.5).circle(55.5).extrude(1.8).translate((0,0,12.7))
    for off in [-36,-18,0,18,36]:
        span=2*math.sqrt(58**2-off**2)
        guard=guard.union(box(fx-span/2,91+off-1.1,12.7,span,2.2,1.8))
    add(f'{i:02}_outlet_guard',guard,(44,49,53))
...
for tag,xx in [('left',0),('right',W-19)]:
    cradle=rb(xx,-G-4,48,19,2*G+8,4,3)
    for yy in [-G-4,G]:
        cheek=box(xx,yy,52,19,4,26).edges('|X').fillet(1.2)
        cradle=cradle.union(cheek)
    add('corner_cradle_'+tag,cradle)
    add('corner_pad_'+tag,rb(xx+1,-T/2,52,17,T,2,2),(174,143,98))
for yy in [-10.8,11.2]:
    # Replaceable compliant seal; bearing loads use end pads, not this lip.
    for a,b in [(24,W/2-.2),(W/2+.5,W-24)]:
...
# XZ workplane extrudes toward -Y: tower lives y=13..20 on the lid side.
...
ycar=ycar.union(box(-32.4,-25+dy,P-19+dz,33,50,5.3))
for yy in [-8+dy,8+dy]:
...
for side,i in [('left','01'),('right','02')]:
    ...
    if side=='left':roof=roof.cut(box(-8,29.7,29,24,7.6,30).val()).clean()
    add(i+'_suction_roof',roof)
deck=take('01_fan_deck').fuse(take('plug_support')).fuse(take('chassis_stop_block')).clean()
add('01_fan_deck_with_plug_support',deck)
skirt=take('01_outlet_skirt').cut(box(-8,29.7,26.4,24,7.6,5).val()).clean()
add('01_outlet_skirt',skirt)

ass=cq.Assembly(name='Precision_5680_D1')
for a in parts:ass.add(a['shape'],name=a['name'],color=cq.Color(*[v/255 for v in a['color']]))
ass.save(str(R/'Precision_5680_D1.step'))
manifest=[]
for a in parts:
...
(R/'geometry.json').write_text(json.dumps({'scope':'Every component is one valid solid. References are simplified; this is not full physical qualification.','parts':manifest},indent=2)+'\n')
print('D1 exported:',len(parts),'valid solids',flush=True)

if __name__=='__main__':
 from render import make_images
 make_images(parts,p)
