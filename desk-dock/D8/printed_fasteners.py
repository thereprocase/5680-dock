"""Coarse, matched, fully printable PETG screw/nut geometry for D8.
...
THREAD_SPEC = {
    "family": "D8 custom trapezoidal matched pair with relieved female root; not ISO metric",
    ...
    "female_minimum_helix_root_gap_mm": 0.02,
    "female_core_boolean_overlap_mm": 0.01,
    "female_standard_total_minor_radius_relief_mm": 0.045,
    "partial_thread_runout_before_thread_mm": 0.8,
    "partial_thread_runout_overlap_mm": 0.2,
...
def _thread_volume(length, diameter, pitch, core_diameter, radial_clearance,
    ...
    if axial_clearance:
        ...
        # Broaden the crest once instead of fusing shifted copies of the
        # helix. Relieve only the minor bore enough to leave a 0.02 mm gap
        # between successive swept turns. The separate 0.01 mm core overlap
        # below avoids putting cylinder/helix intersections on profile
        # vertices, a degeneracy that otherwise produced disconnected solids
        # in OCCT 7.8. Major-diameter and crest clearances stay fixed.
        crest += 2*axial_clearance
        r0=max(r0, r1-(pitch-.02-crest)/2)
        base=crest+2*(r1-r0)
    # Extend and trim both ends: no open/non-manifold helical end faces.
    # Female cutters use one canonical sweep followed by a rigid phase
    # rotation; translated sweep origins needlessly vary the kernel input.
    start=-2*pitch+(0 if axial_clearance else phase_z % pitch)
    helix=cq.Wire.makeHelix(pitch,length+4*pitch,r0,
                            center=cq.Vector(0,0,start))
    overlap=.08
    points=[(r0-overlap,-base/2-overlap),(r1,-crest/2),
            (r1,crest/2),(r0-overlap,base/2+overlap)]
    if axial_clearance:
        points=[(r0-overlap,-base/2),(r0,-base/2),(r1,-crest/2),
                (r1,crest/2),(r0,base/2),(r0-overlap,base/2)]
    profile=cq.Workplane("XZ",origin=(0,0,start)).polyline(points).close()
    ridge=profile.sweep(cq.Workplane(obj=helix),isFrenet=True)
    if axial_clearance:
        ridge=ridge.rotate((0,0,0),(0,0,1),-360*(phase_z % pitch)/pitch)
    core_radius=r0+(.01 if axial_clearance else 0)
    core_solid=cq.Workplane("XY",origin=(0,0,start-pitch)).circle(core_radius).extrude(length+7*pitch)
...
def make_screw(length, diameter=8, pitch=2, head_diameter=18, head_height=5,
               core_diameter=None, phase_z=0, tip_chamfer=.65, thread_length=None):
    ...
    threaded=float(length) if thread_length is None else float(thread_length)
    if not 0 < threaded <= length:
        raise ValueError('Threaded length must be positive and no longer than shank')
    plain=float(length)-threaded
    shaft=cq.Workplane(obj=_thread_volume(threaded,diameter,pitch,core,0.,0.,float(phase_z)-plain)).translate((0,0,plain))
    if plain:
        # A full-major-diameter cylinder overlapping the swept thread crest
        # makes a valid BREP with non-watertight STL seams in OCCT 7.8. Run the
        # smooth shaft down into the thread core instead. The tapered section
        # starts 0.8 mm before the thread and extends 0.2 mm into it, ending
        # inside the solid core; there are no coincident crest surfaces.
        runout=min(.8,plain)
        land=plain-runout
        smooth=cq.Workplane(obj=cq.Solid.makeCone(diameter/2,core/2-.05,
                              runout+.2,cq.Vector(0,0,land)))
        if land:
            smooth=smooth.union(cq.Workplane('XY').circle(diameter/2).extrude(land))
        shaft=shaft.union(smooth)
    if tip_chamfer:
