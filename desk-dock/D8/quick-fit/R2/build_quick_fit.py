"""R2 fit trial from physical R1 feedback; leaves production D8 and R1 intact.
...
def bracket(revised):
    ...
    label = f'R2 GAP {gap:.2f} mm' if revised else f'GAP {gap:.2f} mm'
...
stl = R/'D8-R2-quick-fit-bracket-print-TWO.stl'
step = R/'D8-R2-quick-fit-bracket.step'
...
report = dict(passed=True,revision='R2 fit trial',quantity=2,identical=True,
    ...
    input_sha256={str(p.relative_to(D)):hashlib.sha256(p.read_bytes()).hexdigest()
                  for p in [Path(__file__),R/'fit-parameters.json',D/'parameters.json',D/'contact-profiles.json',D/'geometry.json']},
    output_sha256={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [stl,step]},
    scope='Separate fit prototype. Source visualization mesh and nominal CAD checks only. Vent noise, stability, strength and physical R2 fit untested. Main D8 unchanged.')
...
draw.text((35,20),'D8 R2 fit trial | Print the same bracket twice',font=font(29,True),fill='#20313c')
draw.text((35,1020),f'Inside gap {gap:.2f} mm | 2 mm more lip clearance | Deeper heel cradle',font=font(23),fill='#20313c')
canvas.save(R/'D8-R2-quick-fit.png')
im,_=render([(s,(62,117,139))],(1100,900),(1,-.15,.13),pad=60)
im.save(R/'D8-R2-quick-fit-profile.png')
