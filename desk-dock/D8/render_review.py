...
parser=argparse.ArgumentParser();parser.add_argument('cache',type=Path);parser.add_argument('--views',nargs='+',choices=['assembled','covers','connector','composite'],default=['assembled','covers','connector','composite']);args=parser.parse_args()
...
if 'assembled' in args.views:
 objects=[(a['shape'],a['color']) for a in parts if '_desk_pad_' not in a['name']]
 im,_=render(objects,(1400,1000),(-.55,1,.55),pad=45);im.save(root/'D8-assembled.png')
if 'covers' in args.views:
 objects=[]
 for a in parts:
