...
bounds=[(x,y,z) for x in [-22,build.W+52] for y in [-30,127] for z in [0,build.H+build.p['laptop_depth']+135]]
...
near=[(s,c) for (s,c),a in zip(fixed,[a for a in build.parts if a['name']!='Precision_5680_REFERENCE' and not a['name'].startswith('RUBBER_FOOT_')]) if a['shape'].BoundingBox().xmax>build.W-20 and not 'manifold' in a['name'] and not 'foot' in a['name'] and a['name']!='removable_adjustment_cover']
...
for phase,count,fn in phases:
 for j in range(count):
  ...
  d.text((38,25),'PRECISION 5680  /  D5',font=font(30,True),fill=(37,46,51))
  d.text((39,66),'Your lid-facing view · hinge down · keyboard-left ports at the far end',font=font(19),fill=(96,107,112))
  d.text((1000,139),'PLUG DETAIL / COVER OFF',font=font(19,True),fill=(37,46,51))
  ...
  d.text((1000,691),'2° lean onto fixed guides',font=font(19,True),fill=(37,46,51))
  d.text((1000,723),'Exhaust points 15° upward.',font=font(17),fill=(70,80,85))
  d.text((1000,749),'Open ribs · feet stay clear.',font=font(17),fill=(70,80,85))
  ...
  for fx in build.p['fan_centers_x']:
   pts=project([(fx,106,75),(fx,135,82.77)])[:,:2]+[0,88]
