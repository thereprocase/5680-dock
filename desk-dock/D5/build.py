"""D5 review CAD. Exported +X runs toward the far/plug end in the lid-facing view; +Y lid/fans; -Y underside/intakes; +Z up.
...
for tag,xx,span,top in [('left',6,19,P+12),('right',W-8,28,H+36)]:
    ...
    for sign in [-1,1]:
        pts=[(G,52),(G,top-12),(G+5,top),(G+8,top),(G+3,top-13),(G+3,52)]
        if sign<0:pts=[(-y,z) for y,z in reversed(pts)]
        cheek=cq.Workplane('YZ',origin=(xx,0,0)).polyline(pts).close().extrude(span)
        cheek=cheek.edges('|X').fillet(.8)
        cradle=cradle.union(cheek)
    if tag=='right':
        pts=[(W+20,H+5),(W+20,H+14),(W+24,H+27),(W+27,H+27),(W+23,H+13),(W+23,H+5)]
        fence=cq.Workplane('XZ',origin=(0,8,0)).polyline(pts).close().extrude(16)
        fence=fence.union(box(W+16,-8,48,7,16,4))
        fence=fence.union(box(W+20,-8,52,3,16,H+6-52))
        cradle=cradle.union(fence)
    add('corner_cradle_'+tag,cradle)
# Open underside ribs brace the end supports and stand behind the rubber-foot
# swept envelope. They do not touch the central vent grille or rubber strips.
frame=None
for z in [H+42,H+79]:
    bar=rb(6,-19,z,W+14,4,3,1)
    frame=bar if frame is None else frame.union(bar)
for x in [45,82,119,156,193,230,267,304]:
    rib=rb(x,-19,H+42,3,4,40,1)
    frame=frame.union(rib)
for x in [8,W-5]:frame=frame.union(box(x,-19,49,3,6,H+33))
for x in [8,W-5]:frame=frame.cut(box(x-.1,-15,H+36,3.2,4,50))
add('open_underside_rib_frame',frame,(64,75,81))
for yy in [-12.3,11.0]:
...
# Removable service cover conceals adjustment stages; the cable exits a
# rounded rear slot. Its front clears the laptop throughout engagement.
hood=rb(-52,-36,P-32,51.3,78,57,7)
hood=hood.cut(rb(-49.6,-33.6,P-29.6,54,73.2,52.2,5))
hood=hood.cut(box(-47,29,P-34,42,10,10))
hood=hood.cut(hole((0,0,1),(-34,0,P-34),5,8))
hood=hood.cut(box(-46,-14,P-34,15,44,7))
cable_slot=cq.Workplane('YZ',origin=(-55,0,0)).center(Y,P).slot2D(30,14,90).extrude(10)
hood=hood.cut(cable_slot)
# Lid-side tool-access ports also accept the removable cover's retaining screws.
for z in [P-20,P+17]:hood=hood.cut(hole((0,1,0),(-40,35,z),2,10))
add('removable_adjustment_cover',hood,(47,55,59))
...
for f in contacts['rubber_feet']:
    ...
    foot_keepouts.append(leaned(keep).mirror('YZ',(W/2,0,0)))
# Lean only the laptop, guide/contact parts and plug station; fans retain the
# independent 15 degree upward discharge angle. Contacts carry the 2 degree lean.
for item in parts:
...
rib_frame=take('open_underside_rib_frame')
for side,i in [('left','01'),('right','02')]:
    roof=take(i+'_suction_roof').fuse(take('corner_cradle_'+side)).clean()
    clip=box(-100 if side=='left' else W/2+.15,-100,0,W/2+100 if side=='left' else W,300,400).val()
    roof=roof.fuse(rib_frame.intersect(clip)).clean()
    if side=='left':
        roof=roof.fuse(take('plug_support')).fuse(take('chassis_stop_block')).clean()
    add(i+'_manifold_with_cradle',roof)

# D5 corrects the handedness established by the user's lid-facing photograph.
...
(R/'geometry.json').write_text(json.dumps({'coordinate_frame':p['coordinate_frame'],'scope':'Every component is one valid solid. End contact profiles use Dell mesh sections; nominal closed lid envelope is simplified. References are simplified; this is not full physical qualification.','parts':manifest},indent=2)+'\n')
print('D5 exported:',len(parts),'valid solids',flush=True)
