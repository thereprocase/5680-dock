...
zone=build.box(lo[0],-build.T/2-.12,build.H+lo[2],hi[0]-lo[0],.1,hi[2]-lo[2]).val()
...
d.text((38,27),'D6  /  PLENUM AS THE BACKREST',font=font(31,True),fill=(37,46,51))
d.text((38,74),'Intake region tinted · rubber feet shown in black · laptop leans 5° toward the lid-side guides',font=font(19),fill=(80,94,100))
...
y=615
for title,sub in [('Five Dell case sections','Hinge seat + direct plenum-wall lid bearing'),('Rubber-foot keepout','2 mm expansion; checked along insertion'),('No added structure over the intake',f"{a['rib_net_free_fraction']*100:.1f}% net free over the OEM intake region"),('Bearing load path','Hinge-end seats + plenum-wall liners')]:
