"""D3 review CAD. Exported +X runs toward the far/plug end in the lid-facing view; +Y lid/fans; -Y underside/intakes; +Z up.
...
# Two upright suction pods connect to the low under-hinge manifold.
# Fan plane is 10 degrees from vertical; exhaust points +Y and 10 degrees down.
ANGLE=80
FAN_Y=53; FAN_Z=71
for i,(x0,x1,fx) in enumerate([(-7,W/2,84),(W/2+.30,W+25,269.68)],1):
    ...
    def profile(x,length,inner=False):
        if not inner:
            return (cq.Workplane('YZ',origin=(x,0,0)).moveTo(-24,3)
              .lineTo(-24,41).threePointArc((-21,48),(-13,50)).lineTo(11,50)
              .threePointArc((19,62),(20,85)).lineTo(23,121)
              .threePointArc((40,130),(55.967,131.391))
              .lineTo(35.130,13.214).lineTo(33.329,3).close().extrude(length))
        return (cq.Workplane('YZ',origin=(x,0,0)).moveTo(-21.6,5.4)
              .lineTo(-21.6,40).threePointArc((-18,46),(-12,47.6)).lineTo(11,47.6)
              .threePointArc((21.4,62),(22.4,85)).lineTo(25.4,120)
              .threePointArc((41,127),(53.06,128.27))
              .lineTo(32.64,12.7).lineTo(31.35,5.4).close().extrude(length))
    def low(x,l,rear,bottom,top,front):
        return (cq.Workplane('YZ',origin=(x,0,0)).moveTo(rear,bottom).lineTo(rear,top)
          .lineTo(front+math.tan(math.radians(10))*top,top)
          .lineTo(front+math.tan(math.radians(10))*bottom,bottom).close().extrude(l))
    outer=low(x0,length,-24,3,50,32.8).union(profile(fx-62,124))
    inner=low(x0+2.4,length-4.8,-21.6,5.4,47.6,30.4).union(profile(fx-59.6,119.2,True))
    ...
    roof=roof.cut(fanpose(hole((0,0,1),(fx,91,10),56.5,30)))
    roof=roof.cut(fanpose(hole((0,0,1),(fx,91,10),59,4.7)))
    for xx in [fx-52.5,fx+52.5]:
      for yy in [38.5,143.5]:
        roof=roof.cut(fanpose(hole((0,0,1),(xx,yy,10),2.2,35)))
    add(f'{i:02}_suction_roof',roof)
    for xx in [x0+8,x1-20]:
      for yy in [-20,17]:add(f'{i:02}_foot_{xx:.1f}_{yy}',rb(xx,yy,0,12,16,3,3),(31,34,37))
    fan_start=len(parts)
    ...
    # Discharge finger guard; rotates onto the exposed fan face below.
...
# D3 corrects the handedness established by the user's lid-facing photograph.
...
print('D3 exported:',len(parts),'valid solids',flush=True)
