...
bounds=[(x,y,z) for x in [-52,build.W+22] for y in [-30,75] for z in [0,build.H+build.p['laptop_depth']+135]]
...
near=[(s,c) for (s,c),a in zip(fixed,[a for a in build.parts if a['name']!='Precision_5680_REFERENCE']) if a['shape'].BoundingBox().xmin<20 and not 'manifold' in a['name'] and not 'foot' in a['name']]
...
phases=[('Lower into the guides',24,lambda t:(18,135*(1-smooth(t)))),('Slide 18 mm into the plug',18,lambda t:(18*(1-smooth(t)),0)),('Docked',18,lambda t:(0,0)),('Slide clear of the plug',18,lambda t:(18*smooth(t),0)),('Lift out',24,lambda t:(18,135*smooth(t))),('Ready to dock',12,lambda t:(18,135))]
frames=[];index=0
for phase,count,fn in phases:
 for j in range(count):
  ...
  d.text((38,25),'PRECISION 5680  /  D2',font=font(30,True),fill=(37,46,51))
  d.text((39,66),'Hinge-down slide-in dock · two upright 120 mm fans',font=font(19),fill=(96,107,112))
  d.text((1000,139),'PLUG DETAIL',font=font(19,True),fill=(37,46,51))
  d.text((1000,565),phase,font=font(22,True),fill=(33,107,111))
  d.text((1000,604),f'Slide offset   {x:4.1f} mm',font=font(18),fill=(70,80,85))
  d.text((1000,634),f'Lift above seat   {z:5.1f} mm',font=font(18),fill=(70,80,85))
  d.text((1000,691),'Fans: 10° from vertical',font=font(19,True),fill=(37,46,51))
  d.text((1000,723),'Suction from under the hinge;',font=font(17),fill=(70,80,85))
  d.text((1000,749),'exhaust away from the lid.',font=font(17),fill=(70,80,85))
