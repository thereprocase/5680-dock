...
FONT=next(str(p) for p in [Path('C:/Windows/Fonts/arial.ttf'),Path('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf')] if p.exists())
BOLD=next(str(p) for p in [Path('C:/Windows/Fonts/arialbd.ttf'),Path('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf')] if p.exists())
def font(n,bold=False):
    return ImageFont.truetype(BOLD if bold else FONT,n)

def render(objects,size,camvec,target=None,pad=25,bounds=None,background=None,return_buffers=False):
    ...
    allpts=(np.array(bounds) if bounds is not None else np.concatenate([t.reshape(-1,3) for t,c in meshes]))@basis
    ...
    if background is not None:
        pix,dep,normbuf=[v.copy() for v in background]
    ...
    if return_buffers:return (pix,dep,normbuf),project
