"""D7 review CAD. Exported +X runs away from keyboard-left/plug end; +Y lid/fans; -Y underside/intakes; +Z up.
...
FAN_D=fan_dimensions(p['fan_thickness'])
...
for i,(x0,x1,fx) in enumerate([(-7,W/2,84),(W/2+.30,W+25,269.68)],1):
    ...
    roof=build_fan_service(i,fx,roof,fanpose,box,rb,hole,add,thickness=p['fan_thickness'])
    ...
    for xx in [x0+8,x1-20]:
      for yy in [-20,front_y(3)-20]:add(f'{i:02}_foot_{xx:.1f}_{yy:.1f}',rb(xx,yy,-2.5,12,16,3,3),(31,34,37))
    fan_start=len(parts)
...
(R/'assembly-details.json').write_text(json.dumps({'cassette':cassette_metadata,'body_service':body_service_metadata},indent=2)+'\n')
(R/'geometry.json').write_text(json.dumps({'coordinate_frame':p['coordinate_frame'],'scope':'D7 simplified plenum bearing study. End seats and lid liners support the laptop; no tall cheeks or underside frame. Source-handed USB-C ports. Physical qualification remains outstanding.','parts':manifest},indent=2)+'\n')
print('D7 exported:',len(parts),'valid solids',flush=True)
