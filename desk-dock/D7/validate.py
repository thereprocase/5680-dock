...
for a,b in itertools.combinations(rigid,2):
 ...
 if v>.01:
  ...
  allowed,reason=intended_contact(a,b,build.p,build.leaned,a['shape'].intersect(b['shape']))
  if allowed:designed_contacts.append([a['name'],b['name'],round(v,6),reason])
  else:hits.append([a['name'],b['name'],round(v,3)])
...
back=cq.importers.importStep(str(R/'Precision_5680_D7.step'))
...
data['intentional_compliant_contacts']=designed_contacts
(R/'validation.json').write_text(json.dumps(data,indent=2)+'\n')
