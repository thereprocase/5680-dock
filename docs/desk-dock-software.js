...
export class SoftwareRenderer {
 ...
 render(scene,camera){scene.updateMatrixWorld();camera.updateMatrixWorld();const signature=camera.matrixWorld.elements.join(',')+camera.aspect+scene.children.filter(m=>m.isMesh).map(m=>m.visible+':'+m.matrixWorld.elements.join(',')+':'+m.geometry.uuid+':'+m.geometry.attributes.position.version+':'+m.material.emissive?.getHex()).join(';');if(signature===this.last)return;this.last=signature;
 ...
 scene.traverseVisible(m=>{if(!m.isMesh)return;const g=m.geometry,pos=g.attributes.position;if(!pos)return;let cache=this.cache.get(m);const cacheKey=g.uuid+':'+pos.version+':'+m.quaternion.toArray();
 if(!cache||cache.key!==cacheKey){const idx=g.index?.array||Uint32Array.from({length:pos.count},(_,i)=>i);const shade=new Float32Array(idx.length/3),v=new THREE.Vector3(),a=new THREE.Vector3(),b=new THREE.Vector3();for(let f=0;f<idx.length;f+=3){a.fromBufferAttribute(pos,idx[f]);b.fromBufferAttribute(pos,idx[f+1]).sub(a);v.fromBufferAttribute(pos,idx[f+2]).sub(a);shade[f/3]=.52+.48*Math.max(0,b.cross(v).normalize().transformDirection(m.matrixWorld).dot(light));}cache={key:cacheKey,idx,shade,projected:new Float32Array(pos.count*3)};this.cache.set(m,cache);}
