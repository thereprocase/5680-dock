...
function offset(p){const n=p.name,s=p.center[0]>177?1:-1;if(moving(n))return [18,Math.sin(Math.PI/36)*145,Math.cos(Math.PI/36)*145];
if(n.includes('manifold'))return [s*62,0,0];if(n.includes('foot_'))return [s*62,0,-18];
if(n.includes('removable_plug_cap'))return [-65,15,115];if(n.includes('guard'))return [s*62,130,35];if(n.includes('bezel'))return [s*62,100,27];if(fan(n))return [s*62,60,16];
if(n.includes('lid_bearing'))return [0,-28,0];if(n.includes('seal')||n.includes('corner_pad'))return [s*62,0,35];
if(n.startsWith('Z_'))return [-40,-45,30];if(n.startsWith('Y_'))return [-75,0,30];if(n.startsWith('X_'))return [-110,0,30];return [-85,0,35];}
...
$('state').textContent=e>0?'Exploded assembly':lift>0?'Lift / lower onto the seats':t>0?'Slide clear of the far-side plug':'Docked · lid toward you';
for(const m of meshes){const p=m.userData;m.position.set(...p.explode).multiplyScalar(e);if(moving(p.name)&&!e)m.position.set(x,Math.sin(Math.PI/36)*lift,Math.cos(Math.PI/36)*lift);
m.visible=(!moving(p.name)||$('laptop').checked)&&(!p.name.includes('removable_plug_cap')||$('cover').checked)&&(!fan(p.name)||$('fans').checked);}
air.visible=$('air').checked&&e===0;
}
function view(name){const views={home:[[-300,800,470],[170,35,145]],lid:[[177,930,240],[177,30,155]],under:[[100,-880,410],[177,0,155]],plug:[[-190,190,165],[-12,3,115]]};const [p,t]=views[name];camera.position.set(...p);controls.target.set(...t);controls.update();}
function select(m){if(selected)selected.material.emissive.setHex(0);selected=m;if(m){m.material.emissive.setHex(0x235549);$('part').value=m.userData.name;const n=m.userData.name;let d=moving(n)?'Moving laptop / rubber-foot reference. The two Thunderbolt ports are on the keyboard-left end (screen-right in Lid view). The opposite USB-C port is not Thunderbolt. Source mesh dimensions are approximate.':n.includes('manifold')?'The plenum wall is the laptop backrest. A profiled hinge-end seat is integrated into each half; the underside intake remains unobstructed.':fan(n)?'120 × 15 mm fan assembly, rounded bezel or wire guard. Discharge is 18° above the desk.':n.includes('removable_plug_cap')?'Removable cap of the compact plug cassette. Shims set height; two broad shelf slots allow lateral and insertion calibration before locking.':n.startsWith('Z_')?'Height adjustment and locking hardware: ±5 mm nominal range.':n.startsWith('Y_')?'Lateral adjustment and locking hardware: ±3 mm nominal range.':n.startsWith('X_')?'Two-piece plug cassette: shim height and slide in the shelf slots, then lock the two mounting screws.':n.includes('stop')?'Independent chassis stop. Set it after plug alignment so the connector does not carry the seating load.':'Separate component from the D6 CAD model; see the CAD notes for fit and prototype limitations.';$('detail').textContent=pretty(n)+'. '+d;}}
try{
const [manifest,buffer]=await Promise.all([fetch('models/desk-dock-d6/model.json').then(r=>{if(!r.ok)throw Error(r.status);return r.json();}),fetch('models/desk-dock-d6/model.bin').then(r=>{if(!r.ok)throw Error(r.status);return r.arrayBuffer();})]);
...
for(const x of [84,269.68]){air.add(new THREE.ArrowHelper(new THREE.Vector3(0,Math.cos(Math.PI/10),Math.sin(Math.PI/10)),new THREE.Vector3(x,73,76),75,0x79decd,12,7));air.add(new THREE.ArrowHelper(new THREE.Vector3(0,1,0),new THREE.Vector3(x,-82,128),55,0x80c8ff,10,6));air.add(new THREE.ArrowHelper(new THREE.Vector3(0,0,-1),new THREE.Vector3(x,0,72),32,0xf3b76d,8,5));}
$('loading').hidden=true;window.deskDockReady=true;update();view('home');
}catch(e){$('loading').textContent='Could not load the CAD model. Use the local viewer launcher and reload.';console.error(e);}
...
window.deskDockOrientation=()=>{const c=new THREE.PerspectiveCamera(36,1,1,4000);c.up.set(0,0,1);c.position.set(177,930,240);c.lookAt(177,30,155);c.updateMatrixWorld();const left=new THREE.Vector3(0,0,120).project(c),right=new THREE.Vector3(353.68,0,120).project(c);return {revision:'D6',keyboardLeftScreenX:left.x,keyboardRightScreenX:right.x,keyboardLeftAppearsOnRight:left.x>right.x,withdrawalX:18};};
