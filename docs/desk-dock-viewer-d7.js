...
const handControl=n=>/thumb|knob|handwheel|hand_|push_pin|pivot_pin|printed_chassis_stop/.test(n);
...
let foldAngle=0,foldMotion=null,hinge=null;
...
const hints={
  ...
  service:'Slide-in grilles are hidden. The fan frames remain seated in the body pockets.',
...
function update(){
  ...
  if(viewMode==='service')$('viewHint').textContent=$('guards').checked?'Lift the grille from the open top of the fan pocket. Hide the guards to inspect the fan seating.':$('fans').checked?hints.service:'Fans and grilles are hidden. The open-top pockets and guide rails are exposed.';
  ...
  $('laptop').disabled=foldAngle>0||!!foldMotion;
  $('laptop').parentElement.title=$('laptop').disabled?'Return the holder to ready before showing the seated laptop.':'';
...
function setView(name){
  stop();stopFold();folded=false;foldAngle=0;viewMode=name;$('explode').value=name==='exploded'?65:0;$('motion').value=0;
...
function describe(n){
  ...
  if(n.includes('fan_guard_retainer'))return 'Slides down over the fan in the open-top pocket. Lift it out to service the fan. Friction lands require a PETG fit check.';
...
try{
  const [manifest,buffer]=await Promise.all([
    fetch('models/desk-dock-d7/model.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error(r.status);return r.json();}),
    fetch('models/desk-dock-d7/model.bin',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error(r.status);return r.arrayBuffer();})
  ]);
...
for(const id of ['explode','motion'])$(id).addEventListener('input',()=>{
  stop();stopFold();foldAngle=0;folded=false;
...
$('play').onclick=()=>{
  ...
  stopFold();foldAngle=0;folded=false;viewMode='home';$('explode').value=0;$('laptop').checked=true;
...
requestAnimationFrame(frame);
window.deskDockState=()=>({revision:'D7',parts:meshes.length,visible:meshes.filter(m=>m.visible).length,explosion:+$('explode').value,motion:+$('motion').value,view:viewMode,playing,folded,foldAngle,foldAnimating:!!foldMotion,camLift:camLift(foldAngle),guardsVisible:meshes.filter(m=>guard(m.userData.name)&&m.visible).length,fansVisible:meshes.filter(m=>fan(m.userData.name)&&m.visible).length,positions:meshes.filter(m=>moving(m.userData.name)).map(m=>m.position.toArray())});
